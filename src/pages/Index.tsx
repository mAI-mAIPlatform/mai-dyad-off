...
/* ---------- Helpers ---------- */
// Stub pour la génération d’image (peut être remplacé par une vraie implémentation)
const generateImage = async (prompt: string): Promise<string> => {
  // Retourne une image placeholder
  return `https://via.placeholder.com/400x300?text=${encodeURIComponent(prompt)}`;
};

// Stub pour les instructions de personnalité (peut être enrichi)
...
const Index: React.FC = () => {
  ...
  const handleRegenerateResponse = async (
    ...
    try {
      const apiMessages: OpenRouterMessage[] = messagesToKeep
        .slice(0, -1) // on enlève le dernier message utilisateur (qui sera remplacé)
        .slice(-10)
        .map((msg) => ({
          role: msg.role,
          content: msg.content,
  ...
  const handleSendMessage = async (content: string, isRegeneration = false) => {
    ...
    try {
      // Détection d’une requête d’image
  ...
  const handleNewGhostChat = () => {
    // Implémentation simple similaire à handleNewChat mais avec isGhost
  ...
  const handleUpdateProject = (id: string, name: string, icon: string) => {
    setProjects((prev) =>
      prev.map((p) => (p.id === id ? { ...p, name, icon, updatedAt: new Date() } : p)),
    );
  };

  const handleDeleteProject = (id: string) => {
    setProjects((prev) => prev.filter((p) => p.id !== id));
    // Déplacement des conversations liées vers "null"
    setConversations((prev) =>
  ...
  const handleSelectProject = (id: string | null) => {
    // Filtrage des conversations affichées via ChatSidebar
    // Pas besoin de changer d’état ici
  };
    ...
    <ThemeProvider attribute="class" defaultTheme="system" enableSystem>
      <div className="flex h-screen overflow-hidden bg-background text-foreground">
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          {/* Header avec modèle et paramètres */}
